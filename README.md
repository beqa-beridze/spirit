...
You're root in there but it's proot pretending. You can't touch the Android system and nothing gets loaded into the kernel. systemd is installed but it can't run, because it isn't PID 1 and never will be, so `systemctl` just tells you the system wasn't booted with it. You start everything yourself.
...
One tap on a home screen widget opens Claude Code inside the Fedora. Or it just runs on a timer with nobody touching it at all. That's how I ran a companion agent through most of the summer, waking up every fifteen minutes to deal with whatever had happened while I wasn't looking. That one isn't public and won't be, it knows way too much about my life, but how it was wired up is in [docs/ai-layer.md](docs/ai-layer.md).
...
Bero-OS is the same idea from the other end: there I'm building the machine from source, here the machine was already in my pocket.
...
- adb over loopback doesn't survive a reboot, and nothing on the phone can bring it back. Anything built on adb is fragile, which is half the reason the body app exists.
- No real root, no systemd, no services.
- Only ever tested on this one phone.
